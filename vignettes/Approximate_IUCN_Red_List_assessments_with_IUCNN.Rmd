...
You will see that the output data.frame `labels_train` now contains an extra column with the rescaled labels, and some additional information stored in the column header. Now we can run the `train_iucnn()` function, specifying this time that the want to train a regression model by setting `mode = 'nn-reg'`.
