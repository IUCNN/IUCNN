...
res(ras) <- 1 # the resolution in CRS units, in this case degrees lat/lon
 ...
 # Training features
features <- iucnn_cnn_features(training_occ)

# Prediction features
features_predict <- iucnn_cnn_features(prediction_occ) 
...
## 3. Status prediction
```{r}
pred_data = 'orchid_data/cnn_test/data/cnn_predict_features.rda'
load(pred_data)

pred = iucnn_predict_status(cnn_predict_features,
                             trained_model,
                             target_acc = 0.0
                             )
```
