...
|Geographic bias|`ft_bias`|The resolution of the bias raster (res) or providing a template raster (ras)|
...
### Changing the modelling algorithm
...
You will see that the output data.frame `labels_train` now contains an extra column with the rescaled labels, and some additional information stored in the column header. Now we can run the `train_iucnn()` function, specifying this time that the want to train a regression model by setting `mode = 'nn-reg'`. Further you can add the argument `return_categorical = TRUE` to round the regressed values to integers, which makes it easier to translate the predicted labels for the test set back to numeric IUCN categories:


```{r}
res_4<- train_iucnn(x = features_train,
                    lab = labels_train, 
                    model_name = "iucnn_model_4",
                    mode='nn-reg',
